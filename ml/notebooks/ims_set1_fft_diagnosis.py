"""IMS Set 1 FFT ariza teshisi — hold-out. Set 2 esikleri kilit; retune yok.

NASA (1st_test README): bearing_3 ic bilezik, bearing_4 makara; b1/b2 saglikli.
Kural: `diagnose_fft_bands` varsayilanlari (ADR-0011). Canli `anomaly_events` yok.
"""
...
CACHE_PATH = ROOT / "data" / "ims_set1_fft_bands.npz"
...
def _energy_only(
    ...
    if z_scores[band] < DEFAULT_ABS_Z:
        return False
    others = max(float(energy[name]) for name in BANDS if name != band)
    return float(energy[band]) >= DEFAULT_DOMINANCE * max(others, 1e-12)
...
def _sensitivity_x(store: dict[str, dict[str, np.ndarray]]) -> list[str]:
    ...
    for companion in (5.0, 7.0, 8.0, 10.0, 15.0, 40.0):
...
def write_outputs(store: dict[str, dict[str, np.ndarray]]) -> None:
    ...
    frozen_ok, frozen = protocol_x(
        ...
        companion_z=DEFAULT_COMPANION_Z,
    )
    ...
    (OUT_DIR / "ims_set1_fft_diagnosis.md").write_text("\n".join(lines), encoding="utf-8")
    ...
    axes[0].set_ylabel("z BPFO")
    title = "tuttu" if primary_ok else "TUTMADI"
    axes[0].set_title(f"Set 1 hold-out (X) — Set 2 esikleri {title}")
    axes[1].axhline(DEFAULT_ABS_Z, color="gray", linestyle="--", linewidth=0.8)
    axes[1].set_ylabel("z BPFI (b3 hedef)")
    axes[2].axhline(DEFAULT_ABS_Z, color="gray", linestyle="--", linewidth=0.8)
    axes[2].set_ylabel("z BSF (b4 hedef)")
    axes[3].set_ylabel("teshis")
    axes[3].set_yticks([0, 1, 2, 3], ["belirsiz", "BPFO", "BPFI", "BSF"])
    axes[3].set_xlabel("dosya index")
    figure.tight_layout()
    figure.savefig(OUT_DIR / "ims_set1_fft_diagnosis.png", dpi=120)
    plt.close(figure)
