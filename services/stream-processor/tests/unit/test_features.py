...
def test_square_wave_has_known_features() -> None:
    ...
    assert features == SignalFeatures(rms=1.0, kurtosis=1.0, crest_factor=1.0, peak=1.0)
...
def test_silent_signal_does_not_divide_by_zero() -> None:
    ...
    assert features == SignalFeatures(rms=0.0, kurtosis=0.0, crest_factor=0.0, peak=0.0)
