...
- Girdi: aynı dört özellik (RMS, kurtosis, crest, peak). FFT bantları **çıkarılır ve
  kaydedilir** (ADR-0010) ama bu katmanın vektörüne **girmez**. Teşhis kuralı
  offline (`diagnose_fft_bands`, ADR-0011); canlı `anomaly_events`/`fault_type` yok.
...
- Kapatma: `ML_LAYER_ENABLED=false`. Eşikler warmup eğitim skor niceliği (`ML_WARNING_QUANTILE=0.995`, `ML_CRITICAL_QUANTILE=0.999`; **yalnız Set 2 IF+zarf**, ADR-0008 — başka sette yeniden tarama). Isolation Forest ölçeklenmiş max-norm zarfı eğitim niceliğiyle (çarpan yok). FFT bantları kayda gider, tespite girmez (ADR-0010). FFT teşhis karnesi `ims_set2_fft_diagnosis.md` (ADR-0011, canlı yazım yok). Birim testler sentetik; Set 2 IF karnesi `ims_set2_ml_calibration.md`.
